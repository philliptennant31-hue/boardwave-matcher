    ...
    <div className="rounded-2xl border border-line bg-accent-soft/40 p-5">
      ...
      {open && (
        <div className="mt-4 space-y-4">
          <p className="text-sm leading-relaxed text-ink/80">{weighting.reasoning}</p>
            ...
            {ORDER.map((key) => {
              ...
              return (
                <div key={key} className="flex items-center gap-3 text-xs">
                  <div className="w-28 text-muted">{FACTOR_LABELS[key]}</div>
                  ...
                  <div className="w-8 text-right font-medium text-ink">{w}</div>
                </div>
              )
            })}
          </div>
